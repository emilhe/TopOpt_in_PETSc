...
OC::OC(PetscInt nn, PetscInt mm, PetscInt kk, Vec xo1t,Vec xo2t,Vec Ut,Vec Lt){
    n = nn;
    m = mm;
//    k = kk;
//    if (k<3){
...
OC::OC(PetscInt nn, PetscInt mm,Vec x){
    n = nn;
    m = mm;
...
PetscErrorCode OC::Restart(Vec xo1t,Vec xo2t,Vec Ut,Vec Lt){
...
PetscErrorCode OC::SetOuterMovelimit(PetscScalar Xmin, PetscScalar Xmax, PetscScalar movlim, Vec x, Vec xmin, Vec xmax){
    PetscErrorCode ierr=0;
...
PetscErrorCode OC::Update(Vec xval, Vec dfdx, PetscScalar *gx, Vec *dgdx, Vec xmin, Vec xmax){
    PetscErrorCode ierr=0;
    PetscInt l1, l2;
    PetscScalar move, lmid, xsum;
    PetscInt nloc;
    PetscScalar *xv, *xminv, *xmaxv;
    ...
    while(l2-l1 > 1e-4){
        ...
        for (PetscInt i=0; i<nloc; i++) {
            xv[i] = Max(0.001, Max(xminv[i], Min(1.0, Min(xmaxv[i], xv[i]*sqrt(-dc/lmid)))));
        }
        // Update l1/l2.
        VecSum(xval, &xsum);
        if(xsum - volfrac*n*m > 0){ l1 = lmid; }
        else{ l2 = lmid; }
    }
...
PetscInt OC::Sqrt(PetscScalar d){
    return sqrt(d);
}
